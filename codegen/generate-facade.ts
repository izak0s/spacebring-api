...
for (const path of specPaths) {
  for (const method of HTTP_METHODS) {
    ...
    for (const prop of analyzed.multiProps ?? []) {
      if (!prop.entity) continue;
      entityCandidates.push({
        node,
        root,
        namespace: analyzed.namespace,
        base: entityBase(prop.key),
        expr: prop.entity.schemaRef
          ? componentExpr(prop.entity.schemaRef)
          : `NonNullable<${responseType}["${prop.key}"]>${prop.entity.isArray ? "[number]" : ""}`,
        priority: methodPriority(name),
        opId: op.operationId,
      });
    }
    if (analyzed.bareSchemaRef) {
      entityCandidates.push({
        node,
        root,
        namespace: analyzed.namespace,
        base: entityBase(analyzed.bareSchemaRef),
        expr: componentExpr(analyzed.bareSchemaRef),
        priority: methodPriority(name),
        opId: op.operationId,
      });
    }
...
// Entities can be shared across nodes (same root/base/expr) — dedupe instances.
const allEntities = [...new Set([...entitiesByNode.values()].flatMap((entities) => [...entities.values()]))];
